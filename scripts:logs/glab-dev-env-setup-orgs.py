...
def parse_args():
    ...
    parser.add_argument("-o", "--org",
                    default="",
                    help="organization to fork repos into.\n\n") #TBD - adding organization to clone to - a91e6c
...
def fork(user, repo, dest_org): #TBD - adding organization to clone to - a91e6c
    """
    Fork the repository.
    :param user: user
    ...
    """
    try:
        logging.info("forking the repository " + repo.name + " to " + dest_org.login) #TBD  - adding organization to clone to - a91e6c
        dest_org.create_fork(repo) #TBD - adding organization to clone to - a91e6c
...
def clone_upstream(user, org, repo, dest_org_name): #TBD - setting origin remote - f2c29d
    """
    ...
    try:
        ...
        if dest_org_name: #TBD - setting origin remote - f2c29d
            grb.Repo.remote(local_repo, 'origin').set_url(GITHUB_URL + dest_org_name + "/" + repo.name + ".git") #TBD - setting origin remote - f2c29d
...
def create(g, source, dest_org_name): #TBD - adding organization to clone to - a91e6c
    """
    ...
    for item in REPOS:
        ...
        dest_org = g.get_organization(dest_org_name) #TBD - adding organization to clone to - a91e6c
        fork(user, repo, dest_org) #TBD - adding organization to clone to - a91e6c
        clone_upstream(user, org, repo, dest_org_name) #TBD - setting origin remote - f2c29d
...
def main():
    ...
    * Update the existing forks present in the `sources` folder
    ...
    dest_org_name = str(args.org) if args.org else "" #TBD - adding organization to clone to - a91e6c
    ...
    if args.create:
        create(g, source, dest_org_name) #TBD - adding organization to clone to - a91e6c
    elif args.update:
        update(source) #TBD - adding organization to clone to for update method - a91e6c
    else:
